...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['violation_id'])) {
    ...
    $violationId = pg_escape_string($_POST['violation_id']);
    ...
    if ($result && pg_num_rows($result) > 0) {
        ...
        $violation = pg_fetch_assoc($result);
        ...
        if (pg_query($conn, $insert_sql)) {
            ...
            if (pg_query($conn, $delete_sql)) {
                ...
                echo json_encode(['success' => false, 'error' => 'Failed to delete from t_issues: ' . pg_last_error($conn)]);
            }
        } else {
